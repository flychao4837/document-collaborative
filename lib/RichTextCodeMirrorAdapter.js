    ...
    applyOperation (operation, selection) {
        ...
        for (var i = 0, l = ops.length; i < l; i++) {
            ...
            if (op.isRetain()) {
                ...
                // index 开始位置
                // op.text  插入的字符
                //模拟序列化后的dom的查找 parents(index) -> childs(index) -> childs(index) ->当前鼠标所在节点
                let searchPath = [1,0]//div.root-elem -> span
                let insertOffset = 0
                this.rtcm.insertText(searchPath, insertOffset, op.text);
                index += op.text.length
            } else if (op.isDelete()) {
                console.log("delete text")
                // index 开始位置
                // op.chars 删除的字符长度
                this.rtcm.removeText(op,selection)
            }
        }
