...
class TextOperation {
    ...
    delete(n, attributes) {
        ...
        if (prevOp && prevOp.isDelete()) {
            ...
            this.ops.push(new TextAction('delete', n, attributes))
        }
        return this
    }
    ...
    //示例：插入文字5 [1,"5",3,4] //插入位置，插入的字符，删除,删除的字符长度
    static fromJSON(ops) {
    ...
    apply(str, oldAttributes, newAttributes) {
        ...
        // 在WrappendOPeration.js -> receiveOperation 里this.document = operation.apply(this.document);
        // 服务端初始化时传入的document不一定跟客户端对应，需要同步一个最新的文档。
    ...
    compose(operation2) {
        ...
        var operation1 = this
        if(operation1.targetLength===0){
            //initClientContent ,设置初始值为0，省去由于插入自定义节点的计算长度问题
            operation1.targetLength = operation1.baseLength = operation2.baseLength
        }else if(operation1.targetLength !== operation2.baseLength) {
        ...
        while (true) {
            ...
            if (op1 && op1.isDelete()) {
                operation.delete(op1.chars, op2 && op2.attributes)
                op1 = ops1[i1++]
                continue
            }
            if (op2 && op2.isInsert()) {
            ...
            //----editor添加了自定义节点，起始的baseLength不为0，且插入前一定有retain操作,本地op1为空，codeMirror不用
            if (!op1 && op2.isRetain()) {
                operation.retain(op2.chars, op2.attributes)
                op2 = ops2[i2++]
                continue
            }
            if (!op1 && op2.isDelete()) {
                operation.delete(op2.chars, op2.attributes)
                op2 = ops2[i2++]
                continue
            }
            //----
            ...
            if (op1.isRetain() && op2.isRetain()) {
                ...
                attributes = composeAttributes(op1.attributes, op2.attributes)
                if (op1.chars > op2.chars) {
                    operation['delete'](op2.chars, attributes)
                    op1.chars -= op2.chars
                    op2 = ops2[i2++]
                } else if (op1.chars === op2.chars) {
                    operation['delete'](op2.chars, attributes)
                    ...
                    operation['delete'](op1.chars, attributes)
                    op2.chars -= op1.chars
                    op1 = ops1[i1++]
                }
            } else {
    ...
    shouldBeComposedWithInverted(other) {
        console.log('shouldBeComposedWithInverted')
        if (this.isNoop() || other.isNoop()) {
            return true
        }
    ...
    static transform(operation1, operation2) {
        ...
        console.log('static transform',operation1prime, operation2prime)
