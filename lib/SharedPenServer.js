...
module.exports = class SharedPenServer extends Server {
    ...
    /*服务端根据documen和operation生成一份完整的html*/
    composeDocument(revision) {
        let txt = this.document;
        let steps = [].concat(this.operations);
        let txtOperation = new TextOperation()
        if(revision > steps.length){
            //用户步骤超前，要等待
        }else if(revision > 1){
            for (var i = 0; i < steps.length; i++) {
                //直接合并所有operation是否合适
                txtOperation = txtOperation.compose(steps[i].wrapped)
            }
        }
        console.log('txtOperation',txtOperation);
    }
}