...
class Server {
    ...
    receiveOperation(revision, operation) {
        ...
        //协同规划版本2期，只有单人操作，把文档全文当作一个文本来操作，而且不需要再处理用户鼠标信息
    ...
    class SharedPenServer extends Server {
        ...
        addClient(socket) {
            socket
                ...
                //服务器收到operation，响应操作
                .on('operation', (revision, operation) => {
                    console.log("server-operation",revision,operation)
                    this.mayWrite(socket, (mayWrite) => {
                        if (!mayWrite) {
                            console.log("User doesn't have the right to edit.")
                            return
                        }
                        this.onOperation(socket, revision, operation)
                    })
                })
                //客户端连接断开时需要重新连接（server存在，只是客户端断开来，重新连接改server即可）
                .on('disconnect', () => {
        ...
        onOperation(socket, revision, operation) {
            var wrapped
            
            //Note- TextOperation 生成操作类型及步骤
            try {
                wrapped = new WrappedOperation( TextOperation.fromJSON(operation))
            } catch (exc) {
            ...
            try {
                ...
                console.error(exc)
                this.originErrorNotice(socket, exc)
            }
        }
        ...
        //由于操作错误，导致同步失败，需刷新本地文档，强制同步
        originErrorNotice(socket, message){
