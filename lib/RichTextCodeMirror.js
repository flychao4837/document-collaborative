...
module.exports = class RichTextCodeMirror {
    ...
    onChange(html) {
        ...
        console.log('onChange-鼠标位置信息', selectionData)
    ...
    onClick() {
        //TODO滑动鼠标后在释放鼠标右键，会触发click
        //选中后，再次单击，取到的range仍是上次选中的信息
        this.setUserCursor();
    }
        ...
        if (range.startContainer === range.endContainer) {
            ...
            if (range.startOffset === range.endOffset) {
                ...
                let startLimitOption = {
                    ...
                    nodeListIndex: getNodeIndex(startContainer, rangeRootContainer)
                };
                ...
                startContainerInfo = {
                    ...
                    splitIndex: startLimitOption.splitIndex,
                    domDeep: startLimitOption.domDeep,
                    rootNodeIndex: startLimitOption.rootNodeIndex,
                    nodeListIndex: startLimitOption.nodeListIndex
                };
            } else {
                //TODO -- 鼠标拖选中了文字
                let startContainer = range.startContainer;
                ...
                start = end - distance > 0 ? end - distance : 0;
                startContainerInfo = {
                    nodeType: startLimitOption.nodeType,
                    nodeValue: startLimitOption.nodeValue,
                    splitIndex: startLimitOption.splitIndex,
    ...
    modifyContents(selection, changes) {
        ...
        //先获取到div.root-elem节点
        correctNode = window.jquery(grandNode.children()[path[0]])
        //找到实际变化的子节点
        if (path && path.length) {
            for (var i = 1; i < path.length; i++) {
                correctNode = correctNode.children(i)
            }
        }
        ...
        if (correctNode[0]) {
            if(startContainerInfo.nodeType ==3){
                let textNode = document.createTextNode(insertText)
                correctNode[0].appendChild(textNode)
            }            
        } else {
            alert("节点查找失败");
        }
        this.editor.selection.restoreSelection(); //应用选区
        //更新本地版本this.TextOpHistroy.setCurrentValue(html)
    }
...
function getCharByJsonTree(rootElem, limitOpt) {
    ...
    if (limitOpt) {
        if (limitOpt.domDeep && limitOpt.domDeep.length > 0) {
            let deep = limitOpt.domDeep.splice(1);
            let index = limitOpt.splitIndex;
