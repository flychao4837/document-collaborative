...
module.exports = class RichTextCodeMirror {
    ...
    onChange(html) {
        ...
        console.log('onChange-鼠标位置信息', cursorData)
    ...
    //TODO -- 多层嵌套，且前后标签、文字有类似的结构，取的位置会靠前，而不是实际位置
    //<p class="section">2(这里的2)<span style="font-size: x-small;">2</span><span style="font-weight: bold;"><span style="font-style: italic;">
    // <span style="font-size: x-small;">222</span>222</span></span><span style="font-style: italic;">2</span>2(在里的2跟前面的level，deep都一样，需处理到位)</p>
    getUserCursorInfo() {
        ...
        let startContainerInfo = {
            // nodeName: range.startContainer.nodeName, //节点名
            // nodeValue: range.startContainer.nodeValue,  //节点文本 #Text的值
            // splitIndex: range.startOffset,     //开始位置
            // domDeep: this.getNodeDeepSize(range.startContainer),   //节点相对root的深度
            // nodeType: range.startContainer.nodeType,    //节点类型
            // rootNodeIndex: 1    //段落根节点索引
            // splitIndex: 2    //字符开始或截止位置, range的startOffset、endOffset
        }

        //结束节点信息
        let endContainerInfo = {
            // nodeName: range.endContainer.nodeName, //节点名
            // nodeValue: range.endContainer.nodeValue,  //节点文本 #Text的值
            // splitIndex: range.endOffset,     //结束位置
            // domDeep: this.getNodeDeepSize(range.endContainer),   //节点相对root的深度
            // nodeType: range.endContainer.nodeType,    //节点类型
            // rootNodeIndex: 0    //段落根节点索引
        }
        ...
        if (range.startContainer === range.endContainer) {
            ...
            if (range.startOffset === range.endOffset) {
                ...
                let startLimitOption = {
                    ...
                    domDeep: this.getNodeDeepSize(startContainer),
                    splitIndex: range.startOffset,
                    rootNodeIndex: rootElemsBeforeCursor.prevNodes.length
                };
                getSplitText(rootElemsBeforeCursor, startLimitOption);

                start = end = splitText.length;
                startContainerInfo = startLimitOption;
            } else {
                //TODO -- 鼠标拖选中了文字
                let startContainer = range.startContainer;
                let startLimitOption = {
