...
module.exports = class RichTextCodeMirror {
    ...
    insertText(path, index, text) {
        ...
        if(correctNode[0]){
            let tmp = correctNode.html();
            tmp = tmp.substr(0,index)+text+tmp.substr(index);
            correctNode.html(tmp)
        }else{
            alert("节点查找失败");
        }
        //更新本地版本this.TextOpHistroy.setCurrentValue(html)
    }
    removeText(operation, selection) {
        //TODO -- 根据selection 通过jquery来在指定节点插入数据，用jquery来对editor-html做序列化
        console.log("删除字符")
    }
