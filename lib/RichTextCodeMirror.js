...
module.exports = class RichTextCodeMirror {
    ...
    getCurCursor() {
        ...
        let editorBox = $(".w-e-text-container");
        let boxPosBase = editorBox.offset();
        let curRootContainer = $(cursorData.rangeRootContainer)
        let curRootPosBase = curRootContainer.position();

        let startContainer = $(cursorData.startContainer);
        let endContainer = $(cursorData.endContainer);
        let startPosBase = startContainer.offset();
        let startPosRelative = startContainer.position()
        let endPosBase = endContainer.offset();
        let endPosRelative = endContainer.position();

        console.log("cursorData", cursorData)
        console.log("editorBox", boxPosBase)
        console.log("curRootNode", curRootPosBase)
        console.log("startContainer", startPosBase, startPosRelative)
        console.log("endContainer", endPosBase, endPosRelative)
        let pos = {
            boxPosBase,
            startPosBase,
            startPosRelative,
        ...
        this.setHiddenCursorEditorBox(pos);

    }
    
    //创建隐藏鼠标编辑区域
    setHiddenCursorEditorBox(pos) {
        let $ = this.$;
        let userCursor = $(".userCursor");
        let startTop = pos.startPosBase.top - pos.boxPosBase.top;
        let startLeft = pos.startPosBase.left > pos.startPosRelative.left ? pos.startPosRelative.left - pos.boxPosBase.left : pos.startPosBase.left - pos.boxPosBase.left;
        if (userCursor[0]) {
            userCursor.css({
                'top': startTop,
                'left': startLeft
            ...
            $(".w-e-text-container").append($('<p class="userCursor" style="left:' + startLeft + 'px;top:' + startTop + 'px">|</p>'))
        }
        if ($(".hiddeneEditor")[0]) {

        } else {
            $(".w-e-text-container").append($('<textarea class="hiddeneEditor"></textarea>'))
        }
        //$(".hiddeneEditor").focus();
    }
            ...
            if (unCloseTag().indexOf(tag) > -1) {
                ...
                if (rootElem.childNodes.length > 0) {
                    ...
                    for (var _j = 0, _len2 = rootElem.childNodes.length; _j < _len2; _j++) {
                        ...
                        if (limitOpt && limitOpt.domDeep === 1 && rootElem.childNodes[_j].nodeType === limitOpt.nodeType &&
