...
module.exports =
    class RichTextCodeMirror {
        constructor(codeMirror) {
            ...
            this.dirtyLines_ = []
            this.realtimeTextAttrs_ = {}
            // 格式刷样式
            this.formatBrushAttrs_ = {
                lineAttrs: {},
                spanAttrs: {}
            }
            //是否需要手动刷新页面
        ...
        updateTextAttributes(start, end, updateFn, origin, doLineAttributes) {
            ...
            this.annotationList_.updateSpan(new Span(start, end - start), function(annotation, length) {
                ...
                if (!attributes[AttributeConstants.LINE_SENTINEL] || doLineAttributes) {
                    updateFn(attributes)
                }
            ...
            if (newChanges.length > 0) {
                ...
                setTimeout(this._textAttrsUpdate.bind(this))
            }
        }
        ...
        replaceText(start, end, text, attributes, origin) {
            // 应用来自 server 的 operation 时，先插入纯文本，然后在 codemirror change 事件中应用富文本样式，outstandingChanges_ 对象
            // 用于存储这些富文本属性
            this.changeId_++
        ...
        //插入链接,只能给文字插链接 
        insertLink(tag, placeholder) {
            //添加图片上传容器相关
            var editorWrapper = this.codeMirror.getWrapperElement();
            this.firepadWrapper_ = Utils.elt("div", null, {
                'class': 'xl-upload-container'
            })
            ...
            var cb = function() {
                ...
                //如果没有选中任何文字，则不做替换
                if (self.emptySelection_()){
                    return false;
                }else{
                    let start = self.codeMirror.indexFromPos(self.codeMirror.getCursor('start'))
                    let end = self.codeMirror.indexFromPos(self.codeMirror.getCursor('end'))
                    var text = self.codeMirror.getRange(self.codeMirror.getCursor('start'),self.codeMirror.getCursor('end'))
                    if (src == null || src.substr(0,4).toLocaleLowerCase() !=='http'){
                        alert('请输入带http或https头的完整链接地址');
                    }else{
                        self.insertlinkEntityAt(tag, {
                            'href': src,
                            'target':'_blank',
                            'title': text
                        })
            ...
            var input = Utils.elt('input', null, {
                'class': 'insertimage-dialog-input',
                'id': tag,
                'type': 'text',
            ...
            var buttonsdiv = Utils.elt('div', [submit, cancel], {
                'class': 'insertimage-btn-group'
            });

            var div = Utils.elt('div', [input, buttonsdiv], {
                'class': 'insertimage-dialog-div'
            });
        ...
        insertImage(tag, placeholder) {
            ...
            var cb = function() {
                ...
                if (src !== null){
                    self.newline()
                    self.setLineAttribute(AttributeConstants.LINE_ALIGN, 'center')
                    self.codeMirror.focus()
                    self.insertEntity(tag, {
                        'src': src
                    })
                    self.newline()
        ...
        format() {
            ...
            var line = this.codeMirror.getCursor('head').line
            var _lineAttrs = this.getLineAttributes_(line)
            var _spanAttrs = this.getCurrentAttributes_()
            Utils.shallowClone(_lineAttrs, this.formatBrushAttrs_['lineAttrs'])
            Utils.shallowClone(_spanAttrs, this.formatBrushAttrs_['spanAttrs'])

            // add mouseup event listener
            var _cmWrapper = this.codeMirror.getWrapperElement()
            this._mouseupHandler = () => {
                this._applyFormatBrushIfNeed()
            }
            _cmWrapper.addEventListener('mouseup', this._mouseupHandler)
        }
        _applyFormatBrushIfNeed() {
            // TODO undo/redo 入栈有问题 能否合并成一个？
            //
            // 1: lineAttrs
            var cm = this.codeMirror
            if (!Utils.emptyAttributes(this.formatBrushAttrs_.lineAttrs) && this.emptySelection_()) {
                var line = cm.getCursor('head').line
                this.updateLineAttributes(line, line, (attrs) => {
                    Object.assign(attrs, this.formatBrushAttrs_.lineAttrs)
                })
                this.formatBrushAttrs_.lineAttrs = {}
            }
            // 2: spanAttrs
            if (!Utils.emptyAttributes(this.formatBrushAttrs_.spanAttrs)) {
                if (this.emptySelection_()) { // cursor
                    // 获取光标所在的 span , 然后应用样式
                    var pos = this.codeMirror.getCursor()
                    var index = this.codeMirror.indexFromPos(pos)
                    var spans = this.annotationList_.getSpansForPos(index)
                    for (let span of spans) {
                        this.updateTextAttributes(span.pos, span.end(), (attrs) => {
                            Object.assign(attrs, this.formatBrushAttrs_.spanAttrs)
                        })
                    }
                } else {
                    var _anchorIndex = cm.indexFromPos(cm.getCursor('anchor'))
                    var _headIndex = cm.indexFromPos(cm.getCursor('head'))

                    this.updateTextAttributes(
                        Math.min(_anchorIndex, _headIndex),
                        Math.max(_anchorIndex, _headIndex),
                        (attrs) => {
                            Object.assign(attrs, this.formatBrushAttrs_.spanAttrs)
                        }
                    )
                }
                this.formatBrushAttrs_.spanAttrs = {}
            }

            // remove mouseup event listener
            var _cmWrapper = this.codeMirror.getWrapperElement()
            _cmWrapper.removeEventListener('mouseup', this._mouseupHandler)
        }
        ...
        clearFormat() {
            ...
            if (this.emptySelection_()) {
                ...
                for (let selection of selections) {
                    ...
                    let start = Math.min(_anchorIndex, _headIndex)
                    let end = Math.max(_anchorIndex, _headIndex)
                    this.updateTextAttributes(start, end, (attrs) => {
        ...
        onCursorActivity_() {
            setTimeout(() => {
                this.updateCurrentAttributes_()
                    // 实时文本属性
                this._textAttrsUpdate()
            }, 1)
        }

        //
        _textAttrsUpdate() {
            let realtimeTextAttrs = {}
            Utils.shallowClone(this.getCurrentAttributes_(), realtimeTextAttrs)
            Utils.shallowClone(this.getCurrentLineAttributes_(), realtimeTextAttrs)

            if (!Utils.shallowEqual(this.realtimeTextAttrs_, realtimeTextAttrs)) {
                this.realtimeTextAttrs_ = realtimeTextAttrs
                this.trigger('realtimeTextAttrsChanged', this.realtimeTextAttrs_)
            }
            if(!this.isFreshed){
        ...
        onAnnotationsChanged_(oldNodes, newNodes) {
            ...
            for (i = 0; i < newNodes.length; i++) {
                ...
                if (forLine) {
                    ...
                    var className = this.getClassNameForAttributes_(annotation.attributes).trim()
                    if (className !== '') {
        ...
        updateCurrentAttributes_() {
            ...
            if (anchor > head) { // backwards selection
                ...
                if (pos < this.end()) {
                    pos++
                } // since we're going to look at the annotation span to the left to decide what attributes to use.
            } else {
        ...
        convertCoordinateSystemForChanges_(changes) {
            ...
            function updateIndexFromPos(indexFromPos, change) {
                return function(pos) {
                    if (Utils.posLe(pos, change.from)) {
        ...
        markEntity_(annotationNode) {
            ...
            for (var i = 0; i < annotationNode.length; i++) {
                ...
                //新建的element，对于a标签要重新处理(原计划是选取文字加标签)
            ...
            annotationNode.attachObject({
                ...
                update: function(info) {
                    var entity = Entity.fromAttributes(info)
                    for (var i = 0; i < markers.length; i++) {
        ...
        insertEntity(type, info, origin) {
            this.insertEntityAtCursor(type, info, origin);
        }
        insertEntityAtCursor(type, info, origin) {
            var index = this.codeMirror.indexFromPos(this.codeMirror.getCursor('head'))
            this.insertEntityAt(index, type, info, origin)
        }
        insertEntityAt(index, type, info, origin) {
            var entity = new Entity(type, info)
            this.insertEntity_(index, entity, origin)
        }
        insertEntity_(index, entity, origin) {
            this.replaceText(index, null, EntitySentinelCharacter, entity.toAttributes(), origin)
        }

        //插入的链接是加在文字上的，需要取对应的文字，加标签和属性
        insertlinkEntityAt(type,info,origin){
            var startPos = this.codeMirror.getCursor('start')
            var endPos = this.codeMirror.getCursor('end')
            var text = this.codeMirror.getRange(startPos,endPos)
            var start = this.codeMirror.indexFromPos(startPos)
            var end = this.codeMirror.indexFromPos(endPos)
            var entity = new Entity(type, info)
            this.removeText(start, end, origin)
            this.replaceText(start, null, EntitySentinelCharacter, entity.toAttributes(), origin)
        }
    }