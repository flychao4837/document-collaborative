...
module.exports = class RichTextCodeMirror {
    ...
    setUserCursor() {
        ...
        //this.editor.selection.saveRange(cursorData)
    ...
    //TODO -- 更新选区的startOffset和endOffset
    insertTempContents() {
        let html = this.getCleanHtml();
        let userCursorTmp = '<label class="user-cursor" data-id="'+this.cursorId+'"></label>';
        html = html.substr(0, this.tempRange.start) + this.tempInput + html.substr(this.tempRange.end);
    ...
    clearTempInput() {
        ...
        if (html.length < 8 || html == '<p><br></p>') {
            ...
            if (this.compositionEnd === true) {
                html = html.substr(0, this.tempRange.start - 1) + html.substr(this.tempRange.end);
                this.setEditorContent(html);
                this.cleanTempEditorInput();
                this.onChange(html);
            }
    ...
    updateTempRange(delta, html) {
        ...
        if (delta.removed === "" && delta.inserted !== '') {
            //只增加字符
            this.tempRange.end =  this.tempRange.start = delta.start + delta.inserted.length;
        } else if (delta.removed !== "" && delta.inserted === '') {
            //只减少字符
            this.tempRange.end = this.tempRange.start = delta.start;
            ...
            if(tagMatch && tagMatch[0]){
                ...
                this.tempRange.start = delta.start + tagMatch[0].length + delta.removed.length;
                this.tempRange.end = this.tempRange.start;
            }else{
                //选取来一段文本删除
                this.tempRange.start = delta.start + delta.inserted.length;
                this.tempRange.end = this.tempRange.start;
            }
    ...
    //TOFix -- 在#Text node插入自定义标签导致 #Text被截断，这里需要对同属于统一Text节点的offset做位置合并
    getUserCursorOffset() {
